...
println(templateSet.render(myMarket))
```
...
```
[#ifDefined value=user.nickname alt="(no nickname)" as="nick"]
  Nickname: [=nick]
[/#ifDefined]
```
...
- [=p.name]: [=p.price]
  [#block]Ingredients:
    [#list items=p.ingredients as="i" sep="\n"]
    - [=i.name]
    [/#list]
  [/#block]
[/#list]
